...
g.test_book_get_not_found = function()
    local res = utils.book_get(tostring(1))
...
g.test_book_get_found = function()
    ...
    t.assert_equals(utils.book_get(tostring(1)), {book = test_book_no_shadow, error = nil})
end
...
g.test_book_add_ok = function()
    ...
    local from_space = box.space.books:get(tostring(1))
...
g.test_profile_update_ok = function()
    ...
    t.assert_equals(utils.book_update(tostring(1), changes), {book = changes})
    t.assert_equals(box.space.books:get(tostring(1)), box.space.books:frommap(book_upd))
end

g.test_book_update_not_found = function()
    local res = utils.book_update(tostring(1),{year = 2020})
    res.error = res.error.err
...
g.test_profile_delete_ok = function()
    box.space.books:insert(box.space.books:frommap(test_book))
    t.assert_equals(utils.book_delete(tostring(1)), {ok = true})
    t.assert_equals(box.space.books:get(tostring(1)), nil)
end

g.test_profile_delete_not_found = function()
    local res = utils.book_delete(tostring(1))
    res.error = res.error.err
